#!/usr/bin/env python3
# CLI directly called by running the indra command
...
parser = argparse.ArgumentParser(
    description="Command line interface for indra Authentication.",
    formatter_class=argparse.RawDescriptionHelpFormatter,
    add_help=False,
    prog="indra",
    epilog="For support please visit\nhttps://github.com/gloriousfriedrice/indra")

# Add an argument for the command
parser.add_argument(
    "command",
    help="The command option to execute, can be one of the following: add, disable, enable, remove.",
    metavar="command",
    choices=["add", "config", "disable", "enable", "remove"])
...
builtins.indra_args = args
builtins.indra_user = args.user
...
if os.getenv("SUDO_USER") is None:
    ...
    print("\tsudo indra " + " ".join(sys.argv[1:]))
    sys.exit(1)

# Beond this point the user can't change anymore, if we still have root as user we need to abort
if args.user == "root":
    print("Can't run indra commands as root, please run this command with the --user flag")
    sys.exit(1)
